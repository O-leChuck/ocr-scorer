"""
Unit tests for the interactive folder-selection dialogs.

This module verifies the folder selection and validation behavior,
particularly validate_and_select_folders, which ensures matching file
counts between GT and prediction folders. All tkinter calls are
mocked - these tests never open a real window.
"""
...
from ocr_scorer.dialogs import select_folder, validate_and_select_folders
...
class TestValidateAndSelectFolders(unittest.TestCase):
    ...
    @patch("ocr_scorer.dialogs.select_folder")
    ...
    @patch("ocr_scorer.dialogs.showerror")
    @patch("ocr_scorer.dialogs.select_folder")
    ...
    def test_sticky_defaults_on_retry(self, mock_select):
        ...
        def track_calls(initial_dir, _title):
            """Record the initial_dir passed on each call and return the
            appropriate mismatched/fixed folder for that attempt."""
...
class TestSelectFolder(unittest.TestCase):
    ...
    @patch("ocr_scorer.dialogs.askopenfilename")
    @patch("ocr_scorer.dialogs.Tk")
